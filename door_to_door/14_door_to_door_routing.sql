...
-- ----------------------------------------------------------------------------
-- Business question: what is the best way to move cargo from an origin port
-- to an inland city — e.g. Shanghai → Poltava: via Odesa or via Gdansk?
--
-- For every row in route_requests this ranks the top-3 options end-to-end:
--   sea/rail legs over the shipping_routes graph (recursive CTE, max 3 legs)
--   + inland truck/rail leg from the gateway port (inland_legs.csv)
--   + war-risk surcharge per gateway port (edit port_risk below)
--   + congestion penalty from REAL IMF PortWatch data: a gateway whose
--     container calls are falling year-on-year gets extra buffer days,
--     so rankings re-shuffle automatically when fresh data is fetched.
--
-- Score = USD/CBM all-in + total days × time-value (edit params below).
--
-- Techniques: recursive CTE pathfinding with cycle guard, multimodal join,
--             QUALIFY top-N per group, live-data-driven penalties.
-- ============================================================================

WITH RECURSIVE
params AS (
    SELECT 1.5 AS usd_per_cbm_day        -- value of one day in transit, per CBM
),
...
congestion AS (                          -- real-world buffer days per gateway
    SELECT port_code,
...
paths AS (                               -- sea/rail graph traversal
    SELECT r.origin_port                                    AS origin,
           r.dest_port                                      AS gateway,
           r.origin_port || ' → ' || r.dest_port            AS path,
           [r.origin_port, r.dest_port]                     AS visited,
           r.transit_days + 2                               AS days,  -- +2 origin dwell
    ...
    SELECT p.origin,
           r.dest_port,
           p.path || ' → ' || r.dest_port,
           list_append(p.visited, r.dest_port),
           p.days + r.transit_days + 3,                     -- +3 transshipment dwell
           p.cost + r.cost_per_cbm,
           p.legs + 1
    FROM paths p
    JOIN shipping_routes r ON r.origin_port = p.gateway
    WHERE p.legs < 3
      AND NOT list_contains(p.visited, r.dest_port)         -- cycle guard (LIST, same as module 02)
),
options AS (
